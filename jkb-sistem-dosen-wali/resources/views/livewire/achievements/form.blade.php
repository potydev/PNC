        ...
        <div class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50 z-50">
            <div class="bg-white rounded-lg shadow-lg w-full max-w-lg px-2 py-1">
                <div class="modal-content p-6 max-h-[80vh] overflow-y-auto scrollbar-thin scrollbar-thumb-gray-400 scrollbar-track-gray-200">
                    <h2 class="text-xl font-semibold mb-4">{{ $formTitle }} {{ $achievement->student->user->name ?? '' }}</h2>
                    <form wire:submit="save">
                        ...
                        <!-- SK Penetapan -->
                        <div class="mb-4">
                            <label for="achievementType" class="block text-sm font-medium text-gray-700">Jenis Prestasi / Keaktifan Organisasi</label>
                            <input type="text" wire:model.defer='achievementType' class="mt-1 block w-full border rounded-md p-2">
                            @error('achievementType') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror
                        </div>

                        <!-- SK Penetapan -->
                        <div class="mb-4">
                            <label for="level" class="block text-sm font-medium text-gray-700">Tingkat</label>
                            <select id="level" wire:model.defer='level' name="level" class="mt-1 block w-full border rounded-md p-2">
