...
class SsoAuthController extends Controller
{
    ...
    public function callback(Request $request): RedirectResponse
    {
        ...
        $role = $this->mapRole(data_get($profile, 'role'));
    ...
    {
        if (! is_string($ssoRole)) {
            return 'Mahasiswa';
        }

        $normalized = Str::lower($ssoRole);

        if (str_contains($normalized, 'dosen')) {
            return 'Dosen';
        }
    ...
    {
        $name = trim($baseName) !== '' ? trim($baseName) : 'User SSO';
        $candidate = $name;
        $counter = 1;
